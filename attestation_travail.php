...
if (!$emp) {
    die($lang == 'ar' ? "الموظف غير موجود." : "Employé introuvable.");
}

$sql_prom = "SELECT f.nom_fonction, p.date_promotion 
             FROM promotions p 
             JOIN fonctions f ON p.nouvelle_fonction_id = f.id 
             WHERE p.employee_id = ? 
             ORDER BY p.date_promotion ASC";
...
if ($lang == 'ar') {
    ...
    $label_nom = "الاسم";
    $label_prenom = "اللقب";
    $label_fonction = "الوظيفة الحالية";
    $label_date = "تاريخ الالتحاق";
    $text_final = "قد كان/ت موظفًا لدينا إلى غاية يومنا هذا. سلمت هذه الشهادة للمعني بالأمر لإستعمالها عند الحاجة.";
    $label_history = "الوظائف السابقة في الشركة";
    $date_now_label = "التاريخ";
    ...
    $label_nom = "Nom";
    $label_prenom = "Prénom";
    $label_fonction = "Fonction actuelle";
    $label_date = "Date d'entrée";
    $text_final = "Travaille au sein de l’établissement jusqu’à ce jour. Attestation délivrée sur demande.";
    $label_history = "Fonctions précédentes dans l'entreprise";
    $date_now_label = "Date";
...
<div class="header">
    <img src="/GRH/images/loogo.jpg" style="width:120px; height:auto;">
    <div>'.$company.'</div>
</div>
...
if ($history) {
    ...
    <div class="section">
        ...
        <table class="table">
            <tr>
                <th>#</th>
                <th>'.($lang=='ar' ? "الوظيفة" : "Fonction").'</th>
                <th>'.($lang=='ar' ? "تاريخ الترقية" : "Date promotion").'</th>
            </tr>';
    $i = 1;
